...
*Setup instructions for a gitian build of Nairacoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Nairacoin
...
to the Nairacoin GitHub release page.
...
- [Building Nairacoin Core](#building-nairacoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/nairacoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/nairacoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/nairacoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/nairacoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/nairacoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/nairacoin-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@nairacoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/nairacoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/nairacoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/nairacoin-osx-*-build.assert
```
