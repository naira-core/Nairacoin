...
from test_framework.test_framework import NairacoinTestFramework
...
class ZerocoinSpendTest(NairacoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zNRC_bal, piv_bal):
            zNRC_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zNRC_bal)
            piv_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], piv_bal)
            return zNRC_bal, piv_bal
        ...
        zNRC_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zNRC_balance, 6666)
        ...
        zNRC_balance, balance = check_balances(denom_0, zNRC_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zNRC_balance, balance = check_balances(denom_2, zNRC_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zNRC_balance, balance = check_balances(denom_3, zNRC_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zNRC",
        ...
        _, _ = check_balances(denom_1, zNRC_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
