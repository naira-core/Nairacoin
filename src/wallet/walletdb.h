...
// Copyright (c) 2016-2020 The Nairacoin developers
...
public:
    ...
    bool WriteZNRCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZNRCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZNRCSeed_deprecated(uint256& seed);
    bool EraseZNRCSeed();
    bool EraseZNRCSeed_deprecated();

    bool WriteZNRCCount(const uint32_t& nCount);
    bool ReadZNRCCount(uint32_t& nCount);
