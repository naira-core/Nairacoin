...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NRC at the same time
...
enum ZerocoinSpendStatus {
    ZNRC_SPEND_OKAY = 0,                            // No error
    ZNRC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNRC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNRC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNRC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNRC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNRC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNRC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNRC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNRC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNRC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNRC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNRC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNRC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNRC_SPENT_USED_ZNRC = 14,                      // Coin has already been spend
    ZNRC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNRC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZNRCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzNRCAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzNRCTracker> zNRCTracker;
    ...
    void setZWallet(CzNRCWallet* zwallet);
    CzNRCWallet* getZWallet();
    ...
    /// Get 10000 NRC output and keys which can be used for the Masternode
    ...
    /** zNRC reset */
    boost::signals2::signal<void()> NotifyzNRCReset;
