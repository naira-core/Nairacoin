...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop Nairacoin server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "Nairacoin server stopping";
}
...
/*
        ...
        {"zerocoin", "getzNRCseed", &getzNRCseed, false, false, true},
        {"zerocoin", "setzNRCseed", &setzNRCseed, false, false, true},
        {"zerocoin", "generatemintlist", &generatemintlist, false, false, true},
        {"zerocoin", "searchdzNRC", &searchdzNRC, false, false, true},
        {"zerocoin", "dzNRCstate", &dzNRCstate, false, false, true},
*/
#endif // ENABLE_WALLET
};
...
{
    return "> nairacoin-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(std::string methodname, std::string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:25222/\n";
}
