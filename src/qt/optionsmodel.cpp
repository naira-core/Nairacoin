...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::NRC);
    ...
    if (settings.contains("nAnonymizeNairacoinAmount") || reset)
        SoftSetArg("-anonymizenairacoinamount", settings.value("nAnonymizeNairacoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nairacoin.cpp during shotdown to also remove the window positions
