...
#include "qt/nairacoin/forms/ui_coldstakingwidget.h"
#include "qt/nairacoin/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/nairacoin/requestdialog.h"
...
ColdStakingWidget::ColdStakingWidget(NairacoinGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your NRCs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept NRC delegation / Delegate NRC"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select NRC outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 NRC" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
