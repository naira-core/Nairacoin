...
#include "qt/nairacoin/send.h"
#include "qt/nairacoin/forms/ui_send.h"
#include "qt/nairacoin/addnewcontactdialog.h"
#include "qt/nairacoin/qtutils.h"
#include "qt/nairacoin/sendchangeaddressdialog.h"
#include "qt/nairacoin/optionbutton.h"
#include "qt/nairacoin/sendconfirmdialog.h"
#include "qt/nairacoin/myaddressrow.h"
#include "qt/nairacoin/guitransactionsutils.h"
...
#include "zNRCcontroldialog.h"

SendWidget::SendWidget(NairacoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (NRC)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Nairacoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 NRC");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_NRC : AskPassphraseDialog::Context::Send_zNRC, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zNRC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zNRC selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zNRC transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZNRC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zNRC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zNRC transaction
        ...
        emit message("zNRC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isNRC){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zNRC in your balance to select."));
        }
    }
}
...
void SendWidget::onNRCSelected(bool _isNRC){
    isNRC = _isNRC;
    setCssProperty(coinIcon, _isNRC ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
