...
#include "qt/nairacoin/privacywidget.h"
#include "qt/nairacoin/forms/ui_privacywidget.h"
#include "qt/nairacoin/qtutils.h"
#include "guiutil.h"
#include "qt/nairacoin/denomgenerationdialog.h"
#include "qt/nairacoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(NairacoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zNRC anonymizes your NRC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zNRC or convert back to NRC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 NRC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zNRC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zNRC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zNRC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zNRC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zNRC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zNRC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zNRC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zNRC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zNRC Movements"));
    ...
    // Only Convert to NRC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzNRC->setTitleClassAndText("btn-title-grey", "Total 0 zNRC");
    ui->btnTotalzNRC->setSubTitleClassAndText("text-subtitle", "Show denominations of zNRC owned.");
    ui->btnTotalzNRC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select NRC outputs to mint into zNRC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zNRC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzNRC, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zNRC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of NRC to mint into zNRC"));
    }else{
        btnText = tr("Convert back to NRC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zNRC to convert back into NRC"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzNRC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzNRC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zNRC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zNRC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zNRC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zNRC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zNRC converted back to NRC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any NRC to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNRC </b>";
    ...
    ui->btnTotalzNRC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
