...
#include "qt/nairacoin/forms/ui_dashboardwidget.h"
#include "qt/nairacoin/sendconfirmdialog.h"
#include "qt/nairacoin/txrow.h"
...
DashboardWidget::DashboardWidget(NairacoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of NRC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-piv");
    setCssProperty(ui->labelSquarezPiv, "square-chart-zNRC");
    ...
    ui->labelAmountPiv->setText("0 NRC");
    ...
    ui->btnHowTo->setText(tr("How to get NRC"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZNRC;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            zNRC = (pair.second != 0) ? pair.second / 100000000 : 0;
        ...
        chartData->valueszPiv.append(zNRC);

        int max = std::max(piv, zNRC);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("NRC");
    set1 = new QBarSet("zNRC");
