...
#include "qt/nairacoin/forms/ui_masternodeswidget.h"
#include "qt/nairacoin/qtutils.h"
#include "qt/nairacoin/mnrow.h"
#include "qt/nairacoin/mninfodialog.h"

#include "qt/nairacoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(NairacoinGUI *parent) :
    PWidget(parent),
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:25333 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i NRC requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 NRC required."));
            return;
        }
        showHideOp(true);
