// Copyright (c) 2017-2019 The Nairacoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zNRCwallet.h"
#include "main.h"
#include "txdb.h"
...
CzNRCWallet::CzNRCWallet(std::string strWalletFile)
{
...
bool CzNRCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNRCCount(nCountLastUsed);
    else if (!walletdb.ReadZNRCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNRCWallet::Lock()
{
    seedMaster = 0;
}

void CzNRCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNRCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNRC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNRCWallet::LoadMintPoolFromDB()
{
...
void CzNRCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNRCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNRCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zNRCTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNRCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNRC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zNRCTracker->Add(dMint, true);
...
void CzNRCWallet::SeedToZNRC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNRCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNRCWallet::GenerateDeterministicZNRC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNRCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNRCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNRCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
