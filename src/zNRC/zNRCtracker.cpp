// Copyright (c) 2018-2020 The Nairacoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zNRC/deterministicmint.h>
#include "zNRCtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzNRCTracker::CzNRCTracker(std::string strWalletFile)
{
...
CzNRCTracker::~CzNRCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNRCTracker::Init()
{
...
bool CzNRCTracker::Archive(CMintMeta& meta)
{
...
bool CzNRCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNRCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNRCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNRCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNRCTracker::GetSerialHashes()
{
...
CAmount CzNRCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNRCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNRCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNRCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNRCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNRCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNRCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNRCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNRCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNRCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNRCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNRCWallet* zNRCWallet)
{
    bool iszNRCWalletInitialized = (NULL != zNRCWallet);
    CMintMeta meta;
    ...
    if (! iszNRCWalletInitialized)
        zNRCWallet = new CzNRCWallet(strWalletFile);
    meta.isSeedCorrect = zNRCWallet->CheckSeed(dMint);
    if (! iszNRCWalletInitialized)
        delete zNRCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNRCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNRCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNRCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNRCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNRCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNRCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNRCWallet* zNRCWallet = new CzNRCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNRCWallet);
        }
        delete zNRCWallet;
        LogPrint("zero", "%s: added %d dzNRC from DB\n", __func__, listDeterministicDB.size());
    }
